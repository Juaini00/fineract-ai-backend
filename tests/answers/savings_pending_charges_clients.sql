...
-- is_active=true, outstanding>0) dalam scope kantor terotorisasi, dibatasi
-- row cap 10000 (savings/pending_charges_clients.yaml hard_cap=10000,
-- FIN-133; lihat savings_pending_charges_clients__population.sql). Ditulis
-- independen dari queries/savings/pending_charges_clients.sql: konteks
-- charge->akun->klien->kantor diresolusi lewat CTE charge_context memakai
...
SELECT
    ...
    (CASE
        WHEN cc.charge_due_date IS NULL THEN NULL
        ELSE GREATEST((:'today'::date - cc.charge_due_date)::bigint, 0)
     END) AS days_overdue
FROM charge_context cc
WHERE (SELECT c.office_id FROM m_client c WHERE c.id = cc.client_id) = ANY(:'office_ids'::bigint[])
ORDER BY cc.amount_outstanding_derived DESC, cc.charge_due_date NULLS LAST, cc.savings_account_charge_id
LIMIT 10000
