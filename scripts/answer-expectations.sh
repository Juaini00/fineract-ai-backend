...
# `:today-1y` untuk default relatif (`relative_date` planner).
...
#
# SQL berupa SATU SELECT tanpa titik koma penutup; urutan barisnya harus sama
# dengan urutan jawaban capability (ORDER BY yang sama maknanya).
#
...
for file in "$SQL_DIR"/*.sql; do
    ...
    for token in $(printf '%s' "$declared" | grep -oE '":today-[0-9]+[dmy]"' | sort -u); do
        spec="${token#\":today-}"
        spec="${spec%\"}"
        amount="${spec%?}"
        case "${spec: -1}" in d) unit=days ;; m) unit=months ;; y) unit=years ;; esac
    ...
    # Bungkus menjadi satu dokumen JSON; titik koma penutup dibuang.
    body="$(tail -n +2 "$file" | sed -e 's/[[:space:]]*;[[:space:]]*$//')"
    {
        printf 'SELECT coalesce(json_agg(t), %s) FROM (\n' "'[]'::json"
        printf '%s\n' "$body"
        printf ') t;\n'
    } >"$tmp/q.sql"
