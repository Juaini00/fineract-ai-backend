...
| `SSE_NOTIFICATIONS_ENABLED` | **true** | Notifikasi (`pg_notify` + Redis) hanya memberi tahu "ada yang baru"; kebenarannya selalu `job_events` di PostgreSQL. Mematikannya adalah mode degradasi yang didukung untuk deployment yang tidak mengizinkan `LISTEN`, dan integration test menjalankan folder `sse` dua kali — menyala dan mati — dengan assertion identik. | tidak ada: ini sakelar kapabilitas deployment, bukan angka tuning |
...
SSE_NOTIFICATIONS_ENABLED             = true   # false = hanya fallback polling
